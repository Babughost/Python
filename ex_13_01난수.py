...
print()
print(random.randint(1,10))         # 1이상 10미만의 정수
...
# 0이상 1미만 범위를 백분율로 환산하면 0%이상 100%미만이기 때문에 확률을 처리할 떄도 사용
...
if random.random() < 0.5 :
    print('안녕하세요')             # 임의로 생성된 실수가 0.5보다 적으면 출력


rand = random.randint(1,2)
print(rand)
if rand == 1 :
    print('안녕하세요')             # 1~2사이 정수를 임의로 생성 후 정수가 1이면 출력
...
idx = random.randint(0,3)           # 0~3 사이 임의의 정수를 생성
print(seasons[idx])                 # 



# 5) sample() 함수
# 전달된 시퀀스 자료헹어 속한 요소 중 지정된 개수의 요소를 임의로 반환
# 반환 결과는 리스트 list 자료형이며 중복없이 임의의 요소가 선택
# 중복 없음 => 자료의 인덱스 중복이 없다는 말

print()
print(random.sample(range(1,46), 6))    # lotto 번호 생성 코딩....
...
# 6) shuffle() 함수
# 전달된 시퀸스 자료형에 속한 요소의 순서를 임의로 조정하여 다시 재배치하는 함수
# 호출하면 실제 전달된 시퀸스 자료형의 순서가 재배치
